...
function App() {
  ...
  const loadResults = async (userEmail) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/assessments?email=${encodeURIComponent(
          userEmail
        )}`
      );
  ...
  const startAssessment = async (subject) => {
    try {
      ...
      // Backend already selects 20 random questions.
      // We simply use those 20.
  ...
  const handleOptionClick = async (optionId) => {
    ...
    try {
      ...
      // Save selected option so UI can highlight it.
      setSelectedOption(
        Number(optionId)
      );
      ...
      // Update score only when correct.
      if (correct) {
        setScore(
          (previousScore) =>
            previousScore + 1
  ...
  const finishAssessment = async () => {
    try {
      ...
      /*
       * At this point score already contains the
       * result of every checked question, including
       * the final question because handleOptionClick
       * updates score before this function can run.
       *
       * However React state updates are asynchronous.
       *
       * Therefore we calculate the final score using
       * isAnswerCorrect as well.
       */
      ...
      if (!response.ok) {
        ...
        throw new Error(
          message ||
            "Assessment completed but result could not be saved."
        );
      }
      ...
      /*
       * Even if DB saving fails, show the result.
       */
